...
module.exports = function (argv, cwd) {
    return load('noap', defaults(), argv, cwd);
};

function load(name, defaults, argv, cwd) {
    if (!name) throw new Error('nameless configuration fail');
    if (typeof argv === 'string') {
        cwd = argv;
        argv = null;
    }
    argv = argv || nomnom.parse();
    cwd = cwd || argv.cwd || argv.repo || process.cwd();
    argv.cwd = cwd;
    if (!argv.global) argv.repo = cwd;

    var conf = new nconf.Provider();

    conf.add('env', { type: 'literal', store: fromenv(name + '_') });
    conf.add('argv', { type: 'literal', store: argv });
