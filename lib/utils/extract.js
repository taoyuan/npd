...
function getExtractor(archive) {
    ...
    var type = _.find(extractorTypes, function (type) {
        return utils.endsWith(archive, type);
    });
...
function isSingleDir(dir) {
    return nfn.call(fs.readdir, dir)
        .then(function (files) {
            ...
            return nfn.call(fs.stat, singleDir)
                .then(function (stat) {
    ...
    return nfn.call(fs.readdir, dir)
        .then(function (files) {
            ...
            promises = files.map(function (file) {
                ...
                return nfn.call(fs.rename, src, dst);
            });

            return when.all(promises);
        })
        .then(function () {
            return nfn.call(fs.rmdir, dir);
        });
}
...
function extract(src, dst, opts) {
    ...
    if (!extractor) {
        return when.reject(errors.create('File ' + src + ' is not a known archive', 'ENOTARCHIVE'));
    }

    // Check archive file size
    promise = nfn.call(fs.stat, src)
        .then(function (stat) {
            if (stat.size <= 8) {
                throw errors.create('File ' + src + ' is an invalid archive', 'ENOTARCHIVE');
            }
