...
function download(url, file, options, notify) {
    var operation;
    var response;
    ...
    operation.attempt(function () {
        ...
        .on('response', function (res) {
            ...
            if (status < 200 || status >= 300) {
                return deferred.reject(errors.create('Status code of ' + status, 'EHTTP'));
            }
        ...
        .on('end', function () {
            ...
            if (contentLength && bytesDownloaded < contentLength) {
                req.emit('error', errors.create('Transfer closed with ' + (contentLength - bytesDownloaded) + ' bytes remaining to read', 'EINCOMPLETE'));
            }
        })
