...
exports.getAdmEnvForChildProcess = function() {
    return _.extend(process.env, _.reduce(exports.sorbenv, function(e, value, key) {
        e['ADM_' + key.toUpperCase()] = _.isString(value) ? value : JSON.stringify(value);
        return e;
    }, {}));
};
...
exports.readConfig = function(pkgPath, callback) {
    var p = path.resolve(pkgPath, 'sorb.json');
    if (!fs.existsSync(p)) {
        return callback(new Error(util.format('No sorb.json in pkg path %s.', pkgPath)));
    }
    fs.readFile(p, function(err, content) {
        if (err) {
            return callback(new Error(util.format('Read sorb.json in pkg path %s failed: %j.', pkgPath, err)));
        } else {
            var j = JSON.parse(content);
            log.log('loaded sorb.json of %s.', pkgPath);
            callback(null, j);
        }
    });
};
...
exports.readRegistry = function(callback) {
    var p = path.resolve(exports.admcwd, 'sorb-registry.json');
    if (fs.existsSync(p)) {
