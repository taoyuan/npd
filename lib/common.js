...
exports.getAdmEnvForChildProcess = function() {
    return _.extend(process.env, _.reduce(exports.noapenv, function(e, value, key) {
...
exports.readConfig = function(pkgPath, callback) {
    var p = path.resolve(pkgPath, 'noap.json');
    if (!fs.existsSync(p)) {
        return callback(new Error(util.format('No noap.json in pkg path %s.', pkgPath)));
    }
    fs.readFile(p, function(err, content) {
        if (err) {
            return callback(new Error(util.format('Read noap.json in pkg path %s failed: %j.', pkgPath, err)));
        } else {
            var j = JSON.parse(content);
            log.log('loaded noap.json of %s.', pkgPath);
            callback(null, j);
        }
    });
};
...
exports.readRegistry = function(callback) {
    var p = path.resolve(exports.admcwd, 'noap-registry.json');
    if (fs.existsSync(p)) {
