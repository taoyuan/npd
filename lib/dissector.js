...
function readInstalled(modsdir, transform) {
  var opts = {cwd: modsdir, dot: true};
  var promise = glob('*/.package.json', opts)
    .then(function (filenames) {
      // Foreach package.json found
      return Promise.map(filenames, function (filename) {
        var name = path.dirname(filename);

        return readEndpoint(path.join(modsdir, name));
      });
    });
  if (transform) {
...
exports.toData = function (endpoint, extraKeys, lookup) {
  var names;
  var extra;
  ...
  if (endpoint.extensions) {
    ...
    // by dependency names
    names = Object.keys(endpoint.extensions).sort();
    names.forEach(function (name) {

      // Prevent from infinite recursion when installing cyclic
      // extensions
      if (!_.contains(lookup, name)) {
        data.extensions[name] = exports.toData(
          endpoint.extensions[name],
          extraKeys,
          lookup.concat(endpoint.name)
        );
      }
    }, this);
  }
