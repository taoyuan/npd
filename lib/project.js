        ...
        .then(function (installed) {
            ...
            mout.object.forOwn(installed, function (endpoint, name) {
                promises.push(npm.install(path.resolve(repodir, name)).then(function (result) {
                    return that._linkBins(endpoint.pkgMeta, endpoint.canonicalDir, repodir, that._config.global);
                }));
            });
...
Project.prototype.uninstall = function (names, config) {
    ...
    return this._analyse()
        ...
        .then(function () {
            var promises = [];
            mout.object.forOwn(endpoints, function (endpoint, name) {
                promises.push(that._rmBins(endpoint.pkgMeta, endpoint.canonicalDir, repodir, that._config.global));
            });
            return when.all(promises).then(function () {
