            ...
            return Promise.resolve()
                ...
                .then(function () {
                    ...
                    if (lifecycle.has(modmeta, 'install')) {
                        logger.info('building', 'custom install node modules', pkgmeta);
                        return lifecycle.install(modmeta, folder);
                    }
                    logger.info('building', 'npm install node modules', pkgmeta);
                    return npm.install(folder);
                })
                .then(function () {
                    logger.info('building', 'link stuff', pkgmeta);
                ...
                .tap(function () {
                    logger.info('built', pkgmeta.name, pkgmeta);
                });
        });
    };
}
