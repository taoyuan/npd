...
function build(endpoints, prefix) {
    var that = this;
    var c = npd.config;
    ...
    return promise
        .then(function () {
            ...
            _.forEach(endpoints, function (endpoint, name) {
                ...
                promise = unbuild(endpoint)
                    .catch(function (err) {
                        if (!c.force) throw err;
        ...
        .then(function () {
            ...
            _.forEach(endpoints, function (endpoint, name) {
                ...
                var promise = when.resolve()
                    .tap(function () {
                        logger.debug('build', label, dissector.toData(endpoint));
                    })
                    .then(function () {
                    ...
                    .then(function () {
                        if (fs.existsSync(path.resolve(dir, name, 'node_modules'))) {
                            logger.debug('node_modules', 'exists, skip install ' + label, dissector.toData(endpoint));
                            return when.resolve();
                        }
                        if (lifecycle.has(npdmeta, 'install')) {
                            logger.debug('node_modules', 'custom install ' + label, dissector.toData(endpoint));
                            return lifecycle.install(npdmeta, dst);
                        }
                        logger.debug('node_modules', 'npm install ' + label, dissector.toData(endpoint));
                        return npm.install(dst);
                    })
                    .then(function () {
                        if (c.noBinLinks) return when.resolve();
                        logger.debug('link-bins', label, dissector.toData(endpoint));
                        return linkBins(pkgmeta, endpoint.canonicalDir);
                    })
                    //.then(function () {
                    //    if (npdmeta.extensions) return buildExtensions(npdmeta, dst);
                    //})
                    .then(function () {
        ...
        .finally(function () {
            if (originPrefix) c.prefix = originPrefix;
        })
        .then(function () {
