                ...
                var promise = when.resolve()
                    ...
                    .then(function () {
                        if (fs.existsSync(path.resolve(dir, name, 'node_modules'))) {
                            logger.debug('module-installed', label, dissector.toData(endpoint));
                            return when.resolve();
                        }
                        if (lifecycle.has(npdmeta, 'install')) {
                            logger.debug('custom-install', label, dissector.toData(endpoint));
                            return lifecycle.install(npdmeta, dst);
                        }
                        logger.debug('npm-install', label, dissector.toData(endpoint));
