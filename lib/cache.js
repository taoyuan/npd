        ...
        .then(function (version) {
            ...
            return that._readPkgMeta(canonicalDir)
                .then(function (pkgmeta) {
                    ...
                    return nfn.call(fs.remove, canonicalDir)
                        .then(function () {
...
Cache.prototype.store = function (canonicalDir, pkgmeta) {
    ...
    promise = pkgmeta ? when.resolve(pkgmeta) : this._readPkgMeta(canonicalDir);
    ...
    return promise
        .then(function (pkgmeta) {
            ...
            return nfn.call(fs.stat, dir)
                .catch(function (err) {
                    var lockParams = { wait: 250, retries: 25, stale: 60000 };
                    return nfn.call(lockFile.lock, pkgLock, lockParams).then(function () {
                        ...
                        return nfn.call(fs.stat, dir)
                            .catch(function (err) {
                                ...
                                return nfn.call(fs.mkdirp, path.dirname(dir)).then(function () {
                                    return nfn.call(fs.rename, canonicalDir, dir)
                                        .catch(function (err) {
                ...
                return that._getVersions(sourceId)
                    .spread(function (versions) {
                        if (!versions.length) {
                            ...
                            return nfn.call(fs.remove, path.dirname(dir));
                        }
                    });
            }
        });
...
Cache.prototype.clear = function () {
    return nfn.call(fs.remove, this._dir)
        .then(function () {
            return nfn.call(fs.mkdir, this._dir);
        }.bind(this))
        .then(function () {
            this._cache.reset();
        }.bind(this));
...
Cache.prototype.list = function () {
    ...
    return nfn.call(fs.readdir, this._dir)
        .then(function (sourceIds) {
            promises = sourceIds.map(function (sourceId) {
                return nfn.call(fs.readdir, path.join(that._dir, sourceId))
                    .then(function (versions) {
                        ...
                        if (err.code === 'ENOTDIR' && err.path) {
                            return nfn.call(fs.remove, err.path);
                        }
...
Cache.prototype._getVersions = function (sourceId) {
    ...
    if (versions) {
        return when.resolve([versions, true]);
    }

    dir = path.join(this._dir, sourceId);
    return nfn.call(fs.readdir, dir)
        .then(function (versions) {
