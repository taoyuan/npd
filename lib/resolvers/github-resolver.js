...
var GitRemoteResolver = require('./git-remote-resolver');
var download = require('../utils/download');
var extract = require('../utils/extract');
var errors = require('../errors');

function GitHubResolver(endpoint, config, logger) {
  var pair;

  GitRemoteResolver.call(this, endpoint, config, logger);

  // Grab the org/repo
  // /xxxxx/yyyyy.git or :xxxxx/yyyyy.git (.git is optional)
  pair = GitHubResolver.getOrgRepoPair(this._source);
  if (!pair) {
    throw errors.create('Invalid GitHub URL', 'EINVEND', {
