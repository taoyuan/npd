...
function getConstructor(source, config, registryClient) {
    ...
    if (/^\.\.?[\/\\]/.test(source) || /^~\//.test(source) || path.normalize(source).replace(/[\/\\]+$/, '') === absolutePath) {
        promise = when.nfcall(fs.stat, path.join(absolutePath, '.git'))
            .then(function (stats) {
            ...
            .catch(function () {
                return when.nfcall(fs.stat, path.join(absolutePath, '.svn'))
                    .then(function (stats) {
                        if (stats.isDirectory()) {
                            return function () {
            ...
            .catch(function () {
                return when.nfcall(fs.stat, absolutePath)
                    .then(function () {
                        return function () {
    ...
    return promise
        ...
        .catch(function (err) {
            ...
            return function () {
                return when.nfcall(registryClient.lookup.bind(registryClient), source)
                    .then(function (entry) {
                        if (!entry) {
