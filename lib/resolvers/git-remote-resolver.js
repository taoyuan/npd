...
var GitResolver = require('./git-resolver');
var sh = require('../utils/sh');

function GitRemoteResolver(endpoint, config, logger) {
  GitResolver.call(this, endpoint, config, logger);
...
GitRemoteResolver.prototype._checkout = function () {
  ...
  if (resolution.type === 'commit') {
    promise = this._slowClone(resolution, reporter);
    // Otherwise we are checking out a named ref so we can optimize it
  } else {
    promise = this._fastClone(resolution, reporter);
  }
...
GitRemoteResolver.prototype._slowClone = function (resolution, notify) {
  return sh.exec('git', ['clone', this._source, this._tempDir, '--progress'])
    .then(sh.exec.bind(null, 'git', ['checkout', resolution.commit], {cwd: this._tempDir}, notify));
};

GitRemoteResolver.prototype._fastClone = function (resolution, notify) {
  var branch,
    args,
    that = this;
  ...
  return sh.exec('git', args, {cwd: this._tempDir}, notify) // notify this command ?
    .spread(function (stdout, stderr) {
      ...
      return sh.exec('git', ['checkout', resolution.commit], {cwd: that._tempDir}, notify);
    }, function (err) {
...
GitRemoteResolver.refs = function (source) {
  ...
  value = sh.exec('git', ['ls-remote', '--tags', '--heads', source])
