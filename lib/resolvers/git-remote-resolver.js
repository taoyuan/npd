...
GitRemoteResolver.prototype._slowClone = function (resolution) {
    return sh.exec('git', ['clone', this._source, this._tempDir, '--progress'])
        .then(sh.exec.bind(sh.exec, 'git', ['checkout', resolution.commit], { cwd: this._tempDir }));
};
...
GitRemoteResolver.prototype._fastClone = function (resolution) {
    ...
    return sh.exec('git', args, { cwd: this._tempDir })
        .spread(function (stdout, stderr) {
            ...
            return sh.exec('git', ['checkout', resolution.commit], { cwd: that._tempDir });
        }, function (err) {
