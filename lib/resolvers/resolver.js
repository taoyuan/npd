...
Resolver.prototype._readJson = function (dir) {
    ...
    return nfn.call(fs.readJsonFile, path.resolve(dir, 'package.json'))
        .then(function (json) {
...
Resolver.prototype.hasNew = function (canonicalDir, pkgMeta) {
    ...
    if (pkgMeta) {
        ...
        metaFile = path.join(canonicalDir, '.noap.json');

        promise = nfn.call(fs.readJsonFile, metaFile)
...
Resolver.prototype._savePkgMeta = function (meta) {
    ...
    return nfn.call(fs.writeFile, path.join(this._tempDir, '.bower.json'), contents)
        .then(function () {
