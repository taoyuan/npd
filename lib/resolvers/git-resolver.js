...
function GitResolver(endpoint, config, logger) {
    ...
    // anyway
    fs.mkdirpSync(config.storage.empty);
    process.env.GIT_TEMPLATE_DIR = config.storage.empty;

    Resolver.call(this, endpoint, config, logger);

    if (!hasGit) {
        throw errors.create('git is not installed or not in the PATH', 'ENOGIT');
    }
}
...
GitResolver.prototype._findResolution = function (target) {
    ...
    if ((/^[a-f0-9]{40}$/).test(target)) {
        ...
        return when.resolve(this._resolution);
    }
        ...
        return self.versions(this._source, true)
            .then(function (versions) {
                ...
                return when.all([
                    ...
                    throw errors.create('No tag found that was able to satisfy ' + target, 'ENORESTARGET', {
    ...
    return when.all([
        ...
        .spread(function (branches, tags) {
            ...
            err = errors.create('Tag/branch ' + target + ' does not exist', 'ENORESTARGET');
...
GitResolver.prototype._cleanup = function () {
    ...
    // See: https://github.com/isaacs/fs.remove/issues/19
    if (process.platform === 'win32') {
        return nfn.call(chmodr, gitFolder, 0777)
            .then(function () {
                return nfn.call(fs.remove, gitFolder);
            }, function (err) {
