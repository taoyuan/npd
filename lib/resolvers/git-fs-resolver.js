...
function GitFsResolver(endpoint, config, logger) {
    GitResolver.call(this, endpoint, config, logger);

    // Ensure absolute path
    this._source = path.resolve(this._config.dir, this._source);
}
...
GitFsResolver.prototype._checkout = function () {
    ...
    return this._copy()
        .then(sh.exec.bind(sh.exec, 'git', ['checkout', '-f', resolution.tag || resolution.branch || resolution.commit], { cwd: this._tempDir }))
        // Cleanup unstaged files
        .then(sh.exec.bind(sh.exec, 'git', ['clean', '-f', '-d'], { cwd: this._tempDir }));
};
...
GitFsResolver.refs = function (source) {
    ...
    value = sh.exec('git', ['show-ref', '--tags', '--heads'], { cwd: source })
