...
function UrlResolver(endpoint, config, logger) {
    Resolver.call(this, endpoint, config, logger);

    // If target was specified, error out
    if (this._target !== '*') {
        throw errors.create('URL sources can\'t resolve targets', 'ENORESTARGET');
    }
...
UrlResolver.prototype._hasNew = function (canonicalDir, pkgmeta) {
    ...
    return nfn.call(request.head, this._source, {
...
UrlResolver.prototype._parseHeaders = function (file, response) {
    ...
    if (!disposition) {
        return when.resolve([file, response]);
    }
    ...
    return nfn.call(fs.rename, file, newFile)
        .then(function () {
