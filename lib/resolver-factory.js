...
function createInstance(endpoint, config, logger, registryClient) {
    return getConstructor(endpoint.source, config, registryClient)
        .spread(function (ConcreteResolver, source, fromRegistry) {
            var decEndpointCopy = mout.object.pick(endpoint, ['name', 'target']);
            ...
            if (fromRegistry) {
                ...
                if (!decEndpointCopy.name) {
                    decEndpointCopy.name = endpoint.name = endpoint.source;
                }
            }
...
function getConstructor(source, config, registryClient) {
    ...
    absolutePath = path.resolve(config.dir, source);
    ...
    return promise
        ...
        .catch(function (err) {
            ...
            parts = source.split('/');
            if (parts.length === 2) {
                source = mout.string.interpolate(config.shorthand, {
                    shorthand: source,
                    owner: parts[0],
                    package: parts[1]
                });
