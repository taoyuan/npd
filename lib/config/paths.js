...
// Fallbacks for other operating systems
    ...
    paths.config = paths.config || path.join(base, '.config/noap');
    paths.data = paths.data || path.join(base, '.local/share/noap');
    paths.cache = paths.cache || path.join(base, '.cache/noap');
}

paths.tmp = path.resolve(path.join(tmp, 'noap'));
