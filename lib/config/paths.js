...
// Fallbacks for other operating systems
    ...
    paths.config = paths.config || path.join(base, '.config/npd');
    paths.data = paths.data || path.join(base, '.local/share/npd');
    paths.cache = paths.cache || path.join(base, '.cache/npd');
}

paths.tmp = path.resolve(path.join(tmp, 'npd'));
