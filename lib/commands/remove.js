...
function remove(pkgName, next) {
    noap.load(function (err) {
        if (err) return next(err);

        var p = path.resolve(noap.repo, pkgName);
        ...
        fs.remove(location, function (err) {
            ...
            if (options.keep) {
                ...
                clear(pkgName, function (err) {
                    if (err) {
                        log.error('support dirs and links of %s remove failed: %j.', pkgName, err);
                        return cb(err);
                    }
                    log.log('support dirs and links of %s has been removed', pkgName);
                    cb(null);
                });
            }
        });
...
function clear(pkgName, callback) {
    ...
    // clear support dirs
    rmDirs(support.dirSupports(noap, pkgName));
    callback();
}
